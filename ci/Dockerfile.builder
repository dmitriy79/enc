...
# encocoin_hash
RUN git clone https://github.com/encocoinpay/encocoin_hash
RUN cd encocoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} encocoin
RUN useradd -u ${USER_ID} -g encocoin -s /bin/bash -m -d /encocoin encocoin
...
RUN mkdir /encocoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /encocoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /encocoin-src
